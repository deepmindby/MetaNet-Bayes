"""
Evaluation script for Task-level Variational MetaNet models.

This script provides evaluation capabilities for Task-level Variational MetaNet models,
focusing on accuracy and uncertainty metrics.
"""
...
def find_model_path(model_dir, dataset_name, model_name, debug=False):
    """Find the task-level variational model path for a given dataset"""
    # Base name handling
    ...
    possible_paths = [
        # Primary task variational model paths
        os.path.join(model_dir, val_name, "best_task_variational_model.pt"),
        os.path.join(model_dir, base_name, "best_task_variational_model.pt"),
    ...
    if model_dir != model_specific_dir:
        top_level_paths = [
            os.path.join(model_specific_dir, val_name, "best_task_variational_model.pt"),
            os.path.join(model_specific_dir, base_name, "best_task_variational_model.pt"),
    ...
    raise FileNotFoundError(
        f"Could not find task-level variational model for {dataset_name} (model: {model_name}) in {model_dir}")
...
def evaluate_model(model_path, dataset, device, args):
    """Evaluate task-level variational model on dataset"""
    ...
    analysis_dir = os.path.join(os.path.dirname(model_path), "variational_analysis")
    ...
    if 'config' in state_dict:
        ...
        # Get task-level variational specific parameters
    ...
    # Create the task-level variational model
    try:
        if args.debug:
            print("Creating TaskVariationalMetaNet instance...")
    ...
    # Calculate ECE and get reliability diagram data
    ...
    try:
        ...
        epistemic_uncertainties_np = all_epistemic_uncertainties.cpu().numpy()
        bins = np.linspace(0, max(epistemic_uncertainties_np) * 1.1, 30)

        correct_mask = correct_predictions == 1
        incorrect_mask = correct_predictions == 0

        ax.hist(epistemic_uncertainties_np[correct_mask], bins=bins, alpha=0.5,
                color='green', label='Correct Predictions', density=True)
        ax.hist(epistemic_uncertainties_np[incorrect_mask], bins=bins, alpha=0.5,
                color='red', label='Incorrect Predictions', density=True)

        # Add vertical lines for mean values
        if np.any(correct_mask):
            mean_correct = np.mean(epistemic_uncertainties_np[correct_mask])
            ax.axvline(x=mean_correct, color='green', linestyle='--',
                      label=f'Mean Correct: {mean_correct:.4f}')

        if np.any(incorrect_mask):
            mean_incorrect = np.mean(epistemic_uncertainties_np[incorrect_mask])
        ...
        # Create variational parameters visualization
        var_params = model.get_variational_params()

        # For non-blockwise, show all values
        if not blockwise:
            mean = var_params["mean"].numpy()
            std = var_params["std"].numpy()

            # Create visualization
            fig, ax = plt.subplots(figsize=(12, 6))
            x = np.arange(len(mean))
            ax.bar(x, mean, yerr=std, capsize=4, color='blue', alpha=0.6)

            # Find top coefficients
            top_indices = np.argsort(-np.abs(mean))[:5]

            # Add labels for top coefficients
            for idx in top_indices:
                ax.text(idx, mean[idx] + std[idx] + 0.02, f"{idx}",
                        ha='center', va='bottom', fontsize=10, color='red')

            ax.set_xlabel('Task Vector Index', fontsize=14)
            ax.set_ylabel('Coefficient Value', fontsize=14)
            ax.set_title(f'Task-level Variational Parameters - {dataset_name}', fontsize=16)
            ax.grid(True, alpha=0.3)

            coeffs_path = os.path.join(analysis_dir, f"{dataset_name}_variational_coefficients.png")
            plt.tight_layout()
            plt.savefig(coeffs_path, dpi=300)
            plt.close(fig)
            print(f"Saved variational coefficients plot to {coeffs_path}")

        # For blockwise, create a heatmap
        else:
            mean = var_params["mean"].numpy()
            std = var_params["std"].numpy()
            ...
            # Plot means
            im1 = ax1.imshow(mean, aspect='auto', cmap='coolwarm')
            ax1.set_xlabel('Block Index', fontsize=12)
            ax1.set_ylabel('Task Vector Index', fontsize=12)
            ax1.set_title('Mean Coefficients', fontsize=14)
            fig.colorbar(im1, ax=ax1)

            # Plot standard deviations
            im2 = ax2.imshow(std, aspect='auto', cmap='viridis')
            ax2.set_xlabel('Block Index', fontsize=12)
            ax2.set_ylabel('Task Vector Index', fontsize=12)
            ax2.set_title('Standard Deviations', fontsize=14)
            fig.colorbar(im2, ax=ax2)

            fig.suptitle(f'Task-level Variational Parameters - {dataset_name}', fontsize=16)

            blockwise_path = os.path.join(analysis_dir, f"{dataset_name}_blockwise_parameters.png")
            plt.tight_layout()
            plt.savefig(blockwise_path, dpi=300)
            plt.close(fig)
            print(f"Saved blockwise parameters plot to {blockwise_path}")
    ...
    metrics = {
        ...
        "model_type": "task_level_variational",
...
def main():
    ...
    model_save_dir = os.path.join(args.save_dir, args.model, "evaluation_results_task_variational")
    ...
    # Print configuration
    print(f"\n=== Task-level Variational Evaluation Configuration ===")
    ...
    if summary_results:
        ...
        results_path = os.path.join(
            model_save_dir,
            f"task_variational_results_{args.model}_{timestamp}.json"
        )
        ...
        # Print summary table
        print("\n=== Task-level Variational Evaluation Summary ===")
        print(f"{'Dataset':<15} | {'Accuracy':<10} | {'ECE':<8} | {'Unc. AUROC':<10}")
        print("-" * 55)
        ...
        for result in sorted(summary_results, key=lambda x: x['dataset']):
            print(f"{result['dataset']:<15} | {result['accuracy'] * 100:>8.2f}% | {result['ece']:>6.4f} | "
                  f"{result['uncertainty_auroc']:>9.4f}")

        print("-" * 55)
        print(f"{'Average':<15} | {avg_accuracy * 100:>8.2f}% | {avg_ece:>6.4f} | {avg_auroc:>9.4f}")
        print("=" * 55)
