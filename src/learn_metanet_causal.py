...
def plot_accuracy_and_variance(accuracies, variances, dataset_name, save_dir):
    ...
    Args:
        ...
        variances: list of variance values for each epoch
...
def train(task_vectors, args):
    ...
    train_losses = []
    var_losses = []  # Variance intervention losses for each batch
    val_accuracies = []
    old_params = None
    param_changes = []
    grad_norms = []
    ...
    for epoch in range(args.epochs):
        epoch_loss = 0.0
        epoch_var_loss = 0.0
        # Important: Initialize counters for proper averaging
        total_batches = 0
        var_loss_count = 0  # Count batches with non-zero variance loss
        ...
        for i, batch in enumerate(ddp_loader):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                ...
                if args.causal_intervention:
                    ...
                    # Properly convert to CPU scalar value to avoid CUDA synchronization issues
                    var_loss_cpu = var_loss.detach().cpu().item()

                    # Print actual variance loss in main process
                    if is_main_process() and i % print_every == 0:
                        print(f"Batch {i}, Raw var_loss: {var_loss_cpu:.6f}")
                else:
            ...
            # Record losses - important: use CPU scalars
            task_loss_cpu = task_loss.detach().cpu().item()
            epoch_loss += task_loss_cpu

            # Only count non-zero variance loss to avoid skewing the average
            if var_loss_cpu > 0:
            ...
            if is_main_process():
                ...
                var_losses.append(var_loss_cpu)  # Store actual variance values
            ...
            if (
                ...
                var_str = f", Var Loss: {var_loss_cpu:.6f}" if args.causal_intervention else ""
                print(
                    f"Training epoch: {epoch} [{percent_complete:.0f}% {i + 1}/{num_batches}]\t"
                    f"Task Loss: {task_loss_cpu:.6f}{var_str}\t"
                    f"Data time: {data_time:.3f}\tBatch time: {batch_time:.3f}",
                    flush=True,
                )

        # Record average losses for the epoch - proper handling of the variance loss
        # Only compute average if we have non-zero variance losses
        if total_batches > 0:
            avg_epoch_loss = epoch_loss / total_batches

            # Compute average variance loss only from batches with non-zero values
            if var_loss_count > 0:
            ...
            if is_main_process():
                print(
                    f"Epoch {epoch + 1} average - Task Loss: {avg_epoch_loss:.6f}, "
                    f"Var Loss: {avg_epoch_var_loss:.6f} (from {var_loss_count}/{total_batches} batches)")
    ...
    if is_main_process():
        ...
        if train_losses:
            ...
            monitoring_data = {
                ...
                'var_losses': var_losses,  # Include the actual variance losses per batch
...
if __name__ == "__main__":
    ...
    args.lr = 1e-2  # 1e-2  MetaNet parameters need higher learning rate
